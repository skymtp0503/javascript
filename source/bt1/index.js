...
const checkUsername = () => {  
    ...
    if (!isRequired(fullname)) {
        ...
        let fullnameFormat = fullname.toLowerCase().split(' ').map(word => word.charAt(0).toUpperCase() + word.substring(1)).join(' ');
...
const checkBirth = () => {
    ...
    if (!isRequired(birthdayEl.value)) {
        ...
        if ((dateVal >= dd && mmVal >= mm && yyyyVal >= yyyy) || yyyyVal > yyyy) {
            showError(birthdayEl,`Your date of birth must be less than the current date: ${today}`);
            valid = false;          
        } else {
            valid = true;
...
const checkAll = () => {
    ...
    return checkUsername() && checkEmail() && checkPhone() && checkBirth() && checkPassword() &&checkConfirmPassword();
};
const resetError = () => {
